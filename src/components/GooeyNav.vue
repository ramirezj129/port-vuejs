        ...
        >
          ...
          >
            <a
              ...
              class="outline-none no-underline py-[0.6em] px-[1em] inline-block"
            >
              {{ item.label }}
            </a>
          </li>
...
const emit = defineEmits<{ (e: 'itemClick', index: number): void }>();
...
const handleClick = (e: Event, index: number) => {
  // Prevent the anchor's default "#" jump so we can drive SPA routing.
  e.preventDefault?.();
...
// Keep the active tab in sync with the route (e.g. browser back/forward, or
// deep links). The parent passes the current route index via initialActiveIndex.
watch(
  () => props.initialActiveIndex,
  (next) => {
    if (typeof next === 'number' && next !== activeIndex.value) {
