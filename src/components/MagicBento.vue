    ...
    <div class="gap-2 grid card-responsive">
      <template v-for="(card, index) in cardData" :key="index">
        ...
        >
          ...
          <div v-if="card.body" class="card__body" v-html="card.body"></div>
        ...
        >
          ...
          <!-- body renders raw markup (e.g. Font Awesome <i> icons) -->
          <div v-if="card.body" class="card__body" v-html="card.body"></div>
          <div class="relative flex flex-col text-white card__content">
            <h3
              :class="`card__title font-normal text-base m-0 mb-1 ${textAutoHide ? 'text-clamp-1' : ''}`"
...
/* Card body — holds raw markup such as Font Awesome icons */
.card__body {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
