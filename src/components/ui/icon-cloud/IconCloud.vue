...
import type { SphereIcon } from "./index";
import { onBeforeUnmount, onMounted, reactive, ref, watchEffect } from "vue";

// Props interface is defined inline (not imported) because Vue's SFC compiler
// needs `typescript` installed to resolve imported types in defineProps — this
// project has no TS dep, so keeping it local avoids that requirement.
interface IconCloudProps {
  class?: HTMLAttributes["class"];
  images?: string[];
  /** Canvas width/height in px. The sphere scales with it. */
  size?: number;
}

// Small local class-merge helper so we don't need @inspira-ui/plugins.
function cn(...classes: Array<string | undefined | null | false>) {
  return classes.filter(Boolean).join(" ");
}

const props = withDefaults(defineProps<IconCloudProps>(), { size: 300 });

const { images } = props;

// Sphere radius derived from canvas size (0.32 keeps the original look at
// size=300, where radius ≈ 96). All projection math below is relative to this.
const sphereRadius = props.size * 0.32;
...
function handleMouseDown(e: MouseEvent) {
  ...
  imagePositions.value.forEach((icon) => {
    ...
    const scale = (rotatedZ + 2 * sphereRadius) / (3 * sphereRadius);
  ...
  function animate() {
    ...
    imagePositions.value.forEach((icon, index) => {
      ...
      const opacity = Math.max(
        0.2,
        Math.min(1, (rotatedZ + 1.5 * sphereRadius) / (2 * sphereRadius)),
      );
